...
public final class FactoringUtility {
    ...
    /**
     * Private conzstructor so this utility class cannot be instantiated.
     */
    ...
    public static int aNonTrivialFactorV1(int n) {
        ...
        while (!found) {
            if (n % factor == 0) {
                ...
                factor = factor + 2; //only find result*[odd * 2] from 3
            }
        }
        return factor;
    }
    ...
    public static int aNonTrivialFactorV3(int n) {
        assert n > 2 : "Violation of: n > 2";
        int factor = 0; // starting from zero - divide by zero
        boolean found = false;
        while (!found) {
            if (n % factor == 0) {
                found = true;
