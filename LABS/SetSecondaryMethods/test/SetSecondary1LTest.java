...
public final class SetSecondary1LTest {
    ...
    @Test
    public final void testIsSub2() {
        Set<String> s1 = createFromArgs("one", "two");
        Set<String> s2 = createFromArgs("one", "two", "three");
        boolean result = s1.isSubset(s2);
        assertEquals("{one,two}", s1.toString());
        assertEquals("{one,two,three}", s2.toString());
        assertTrue(result);
    }

    @Test
    public final void testIsSub3() {
        Set<String> s1 = createFromArgs("one", "two");
        Set<String> s2 = createFromArgs("one");
        boolean result = s1.isSubset(s2);
        assertEquals("{one,two}", s1.toString());
